...
const RegisterRequirementsUser = forwardRef<HTMLFormElement, IRegisterRequirementsUser>(({ onSubmit, showSubmit }, ref) => {
  const { formData, setFormData } = useRegisterStore()
  const { register, handleSubmit, formState, setValue, reset } = useFormContext<TRequirementsUser>()
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)} className='regRequirements-form' ref={ref}>
      {Object.entries(keysValues).map(([key, field]) => (
        <section className={`regRequirements-section ${switchClass(err[field], 'error')}`} key={key}>
      ...
      <div className='regRequirements-form__actions'>
        {showSubmit && (
          <button type='submit' className={`regRequirements-form__submit btn bgr ${switchClass(!isValid, 'inactive')}`}>
